...
for i in range(100):
    ...
    x3 = i + 2
    x4 = i / 3
    y = 5.2 * x1 - 3.8 * x2 + 7.5 * x3 + 2.1 * x4 + 4.7
    y += np.random.normal(0, 0.3) # Adding some noise
    data.append(y)
...
def train_model(model, X_train, y_train, X_val, y_val,
    ...
    for epoch in range(1, epochs + 1):
        ...
        optimizer.step()
        if epoch % 5000 == 0:
            print(f"Epoch {epoch}, current LR: {scheduler.get_last_lr()[0]:.6f}") # Print current learning rate
        scheduler.step()
...
x3 = next_x + 2
x4 = next_x / 3
correct_term = 5.2 * x1 - 3.8 * x2 + 7.5 * x3 + 2.1 * x4 + 4.7
